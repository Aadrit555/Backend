...
async def get_experiment_status(experiment_id: str):
    """Fetch real-time training status and logs for an experiment."""
    status_path = settings.experiments_dir / experiment_id / "status.json"
...
async def get_project(project_id: str):
    ...
    try:
        ...
        for e in experiments:
            ...
            ds = next((d for d in datasets_rows if d.id == e.dataset_id), None)
            dataset_name = ds.name if ds else None
            
            if not dataset_name:
                valid_ds = [d for d in datasources if d.project_id == project_id and d.uploaded_at <= e.created_at]
                if not valid_ds:
                    valid_ds = [d for d in datasources if d.project_id == project_id]
                
                if valid_ds:
                    valid_ds.sort(key=lambda d: d.uploaded_at)
                    dataset_name = valid_ds[-1].original_filename
                else:
                    dataset_name = e.model_name
...
def download_gguf_model(experiment_id: str):
    """Download generated GGUF artifact(s) for an experiment."""
    import zipfile
    ...
    try:
        ...
        search_roots = [settings.experiments_dir / experiment_id / "export"]

        from backend.db import ModelArtifact, TrainingRun
        ...
        if run:
            ...
            if artifact:
                ...
                search_roots.extend([artifact_path, artifact_path.parent])
        ...
        for root in search_roots:
            if not root.exists():
                continue
            candidates = [root] if root.is_file() and root.suffix.lower() == ".gguf" else root.rglob("*.gguf")
        ...
        if not gguf_files:
            raise HTTPException(
                status_code=404,
                detail="GGUF artifact not found. Re-run fine-tuning with GGUF export enabled.",
            )

        gguf_files.sort(key=lambda p: p.name)
        if len(gguf_files) == 1:
        ...
        zip_path = settings.experiments_dir / experiment_id / f"{experiment_id}_gguf.zip"
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
            for gguf_path in gguf_files:
                zip_file.write(gguf_path, arcname=gguf_path.name)

        return FileResponse(
            path=str(zip_path),
            filename=f"gguf_{experiment_id[:8]}.zip",
            media_type="application/zip",
        )
    finally:
        db.close()
...
async def predict_vision(
    ...
    try:
        if is_video:
            import tempfile
            with tempfile.NamedTemporaryFile(suffix=Path(filename).suffix, delete=False) as tmp_vid:
        ...
        return {
            ...
            "speed_ms": getattr(first_res, "speed", {}),
            "is_video": is_video,
        }
    except Exception as e:
        import traceback
...
_active_tabular_model = None
_active_tabular_path = None

@router.post("/api/model/inspect")
async def api_model_inspect(req: InspectRequest):
    global _active_tabular_model, _active_tabular_path
    try:
        from autogluon.tabular import TabularPredictor
        if _active_tabular_model is None or _active_tabular_path != req.model_path:
            _active_tabular_model = TabularPredictor.load(req.model_path)
            _active_tabular_path = req.model_path
        
        features = _active_tabular_model.feature_metadata_in.get_features()
        label = getattr(_active_tabular_model, "label", "target")
        problem_type = getattr(_active_tabular_model, "problem_type", "unknown")
...
async def api_model_predict_tabular(req: PredictTabularRequest):
    global _active_tabular_model, _active_tabular_path
    import pandas as pd
    try:
        model_path = str(settings.models_dir / req.experiment_id)
        from autogluon.tabular import TabularPredictor
        if _active_tabular_model is None or _active_tabular_path != model_path:
            _active_tabular_model = TabularPredictor.load(model_path)
            _active_tabular_path = model_path
            
        df = pd.DataFrame([req.features])
        prediction = _active_tabular_model.predict(df)
